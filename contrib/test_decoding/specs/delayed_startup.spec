...
session "s2"
setup { SET synchronous_commit=on; }
step "s2init" {SELECT 'init' FROM create_decoding_replication_slot('isolation_slot', 'test_decoding');}
step "s2start" {SELECT data FROM decoding_slot_get_changes('isolation_slot', 'now', 'include-xids', 'false');}
