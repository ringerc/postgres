...
/*
 * Use the test_protocol_headers handler's "test." prefix and pick
 * test.txn_scope --- the test module registers a single handler under
 * "test." and internally demultiplexes to per-key lifetimes; the
 * txn_scope key is the one cleared at COMMIT, matching the
 * transaction-scoped logging this test asserts on.
 */
#define TEST_KEY		"test.txn_scope"
...
usage(const char *argv0)
{
	fprintf(stderr,
			...
			"  attach <value>             attach test.txn_scope=<value>, run SELECT, print 'ok'\n"
			...
			"  defer_after_fail <value>   attach, send call that fails validation, clear, SELECT; print 'ok'\n"
...
main(int argc, char **argv)
{
	...
	else
	{
		...
		else if (strcmp(mode, "defer_after_fail") == 0)
		{
			/*
			 * Verify the deferred-flush property: when a PQsend* call
			 * fails its own argument validation, queued headers must
			 * remain queued (not silently consumed onto the wire).
			 *
			 *  1. Attach a header.
			 *  2. Call PQsendQuery(NULL) --- this returns 0 after the
			 *	  null-command check, BEFORE any protocol bytes go out.
			 *	  With the deferred-flush fix the header is still in
			 *	  the queue; without the fix it has already been sent
			 *	  as a stray 'M' frame.
			 *  3. Discard the queue with PQclearHeaders.
			 *  4. Run a normal SELECT --- with the fix in place no 'M'
			 *	  frame is sent on the wire at any point.
			 *
			 * The TAP test then asserts that no test_protocol_headers
			 * set log line appears in the slice.  Under the bug, a set
			 * line would appear at step 2.
			 */
			if (argc != 4)
				usage(argv[0]);
			if (!PQattachHeader(conn, TEST_KEY, argv[3]))
				die_connerr(conn, "PQattachHeader");
			if (PQsendQuery(conn, NULL) != 0)
			{
				fprintf(stderr,
						"libpq_headers: PQsendQuery(NULL) unexpectedly succeeded\n");
				PQfinish(conn);
				return 1;
			}
			PQclearHeaders(conn);
