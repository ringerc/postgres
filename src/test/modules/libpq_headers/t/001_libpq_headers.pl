...
#
# The libpq_headers helper binary attaches the test.txn_scope header
# and runs a no-op SELECT to drive the protocol forward.  Server-side
# verification uses the test_protocol_headers loadable module, which
# registers a single handler for the "test." prefix and demultiplexes
# to per-key lifetimes; test.txn_scope is the transaction-scoped key
# whose set/clear events get logged.  The TAP test reads the log
# between known offsets to confirm:
#
...
# Helper: count occurrences of /set scope=transaction key=test\.txn_scope
...
sub count_sets
{
	...
	my $re = qr/test_protocol_headers: set scope=transaction key=test\.txn_scope value=\Q$value\E/;
	return scalar(() = $log =~ /$re/g);
}

# Helper: count occurrences of any test_protocol_headers set line under
# the test. prefix, ignoring the value.  Used for absence checks
...
sub count_any_sets
{
	...
	my $log = PostgreSQL::Test::Utils::slurp_file($node->logfile, $offset);
	return scalar(() =
		$log =~ /test_protocol_headers: set scope=transaction key=test\./g);
}
...

{
	...
	# log writer.
	$node->wait_for_log(
		qr/test_protocol_headers: set scope=transaction key=test\.txn_scope value=\Q$VALUE\E/,
		$offset);
...
# ----------------------------------------------------------------------
# Group 7: deferred flush --- a sender that fails its own argument
# validation must NOT drain the header queue.  The libpq client
# attaches a header, calls PQsendQuery(NULL) (which returns 0 after
# argument validation), discards the queue with PQclearHeaders, and
# then runs a normal SELECT.  The server must observe no
# RequestHeaders frame across the whole exchange.
#
# Under the previous behaviour (eager flush in PQsendQueryStart), the
# 'M' frame would have been serialized onto the wire before
# PQsendQuery's null-command check fired, producing a stray set log
# line ahead of any successful command.
# ----------------------------------------------------------------------

{
	my $offset = -s $node->logfile;
	my ($rc, $out, $err) = run_client('defer_after_fail', $VALUE);
	is($rc, 0, 'defer_after_fail mode exited 0');
	is($out, "ok\n", 'defer_after_fail mode reported ok');

	# Barrier: drive a marker query so any pending log writes flush.
	$node->safe_psql('postgres', 'SELECT 1');
	is(count_any_sets($offset), 0,
		'failed-validation send does not drain the header queue (no stray M)');
}
