...
/*
 * Search for a free slot and mark it as used and active.
 */
void
...
/*
 * Find an previously initiated slot and mark it as active.
 */
void
...
ReplicationSlotRelease(void)
{
	 ...
	 * marked inactive after a crash/restart anyway.
	 */
	SaveSlot(slot);
...
/*
 * Permanently remove a replication slot.
 */
void
ReplicationSlotDrop(const char *name)
{
	...
	if (slot->active)
	{
		...
		ereport(ERROR,
				(errcode(ERRCODE_OBJECT_IN_USE),
				 errmsg("could not free active replication slot \"%s\"", name)));
	}
...
CheckSlotRequirements(void)
{
	...
	if (wal_level < WAL_LEVEL_ARCHIVE)
		ereport(ERROR,
				(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
				 errmsg("slot usage requires wal_level>=archive")));
}
...
ReplicationSlotSave()
{
	...
	sprintf(path, "pg_llog/%s", NameStr(MyReplicationSlot->name));
	SaveSlotGuts(MyReplicationSlot, path);
}

/*
 * Load replication slots from disk into memory at server startup. This needs
 * to be run before we start crash recovery.
 */
void
StartupReplicationSlots(XLogRecPtr checkPointRedo)
{
	...
	replication_dir = AllocateDir("pg_llog");
	while ((replication_de = ReadDir(replication_dir, "pg_llog")) != NULL)
	{
			...
			strcmp(replication_de->d_name, "old") == 0)
		{
			...
			sprintf(path, "pg_llog/%s", replication_de->d_name);
